...
#include <unistd.h>
#include "patch_dirtypipe.skel.h"
...
{
    struct patch_dirtypipe_bpf *skel;
    int err;
    ...
    skel = patch_dirtypipe_bpf__open_and_load();
    if (!skel) {
    ...
    err = patch_dirtypipe_bpf__attach(skel);
    if (err) {
...
cleanup:
    patch_dirtypipe_bpf__destroy(skel);
    return err;
}
